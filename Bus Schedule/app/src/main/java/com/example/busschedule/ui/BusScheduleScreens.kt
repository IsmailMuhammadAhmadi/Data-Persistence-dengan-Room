...
fun BusScheduleDetails(
    ...
    var mainStreetEncountered = false // Flag to track the first "Main Street"
    ...
    LazyColumn(
        ...
        items(
            ...
            // Check if this item is "Main Street"
            if (schedule.stopName == "Main Street") {
                if (mainStreetEncountered) {
                    // If "Main Street" has already been encountered, add a boundary
                    Divider(
                        modifier = Modifier.padding(vertical = 8.dp),
                        color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.5f)
                    )
                }
                mainStreetEncountered = true // Mark that we have encountered "Main Street" once
            }
        ...
        TopAppBar(
            ...
            navigationIcon = {
                IconButton(onClick = onBackClick) {
                    // Replace with your custom image for the back icon
                    Icon(
                        painter = painterResource(id = R.drawable.ic_action_bus),  // your image resource
